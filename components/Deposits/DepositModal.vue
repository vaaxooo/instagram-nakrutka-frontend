<template>
    <div class="modal fade" :class="{'show d-block': isVisible}" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-dialog modal-dialog-centered">
            ...
            <div class="modal-content">
                <div class="modal-header px-0">
                    ...
                    <button type="button" class="btn-close-modal" @click="isVisible = !isVisible">
                        <span class="material-icons icon icon-close">cancel</span>
                    </button>
                </div>

                <!-- <div v-if="isLoading" class="loader-content text-center">
                    <div class="loader-content-text">
                        <div class="loader-details loader">
                            <span class="material-icons loader">
                ...
                <div class="modal-body px-0">
                    <div class="balance" v-if="payment_method === null && withdrawal_method === null && show_history === false">
                    ...
                    <deposits-deposit-modal-navigation :current_page="current_page" v-if="payment_method === null && withdrawal_method === null && show_history === false" />

                    <deposits-deposit-methods v-if="current_page === 'deposit' && payment_method === null && withdrawal_method === null" :assets="currencies.assets" />
                    <deposits-withdrawal-methods v-if="current_page === 'withdrawal' && payment_method === null && withdrawal_method === null" />
                    <deposits-balance-list v-if="current_page === 'balances'" :balances="balances" />
                    <deposits-history-transactions v-if="current_page === 'history' && show_history === false" />


                    <!-- DEPOSIT STEPS -->
                    <DepositStepTwo v-if="payment_method === 'card' && step === 0" />
                    <DepositStepThree v-if="payment_method === 'card' && step === 1" :deposit="deposit" />
                    <DepositStepFinal v-if="payment_method === 'card' && step === 2 && depositResponse !== null" :deposit="depositResponse" />
                    <DepositStepCrypto v-if="payment_method === 'crypto' && step === 0 && depositResponse !== null" :deposit="depositResponse" />
                    <!-- DEPOSIT STEPS END  -->

                    <!-- WITHDRAWAL STEPS -->
                    <WithdrawalStepOne v-if="withdrawal_method === 'card' && step === 0" />
                    <!-- WITHDRAWAL STEPS END  -->

                    <!-- HISTORY DETAILS -->
                    <HistoryDetails v-if="show_history" :history_hash="history_hash" />
                    <!-- HISTORY DETAILS END -->
                </div>
            </div>
        </div>
...
<script>
import DepositStepTwo from "./DepositSteps/DepositStepTwo.vue";
import DepositStepThree from "./DepositSteps/DepositStepThree.vue";
import DepositStepFinal from "./DepositSteps/DepositStepFinal.vue";
import DepositStepCrypto from "./DepositSteps/DepositStepCrypto.vue";
import WithdrawalStepOne from "./WithdrawalSteps/WithdrawalStepOne.vue";
import HistoryDetails from "./History/HistoryDetails.vue";
...
export default {
    ...
    mounted() {
        ...
        const eventHandlers = {
            ...
            'switchModalPage': (data) => {
                this.current_page = data;
            },
            'setPaymentMethod': (data, asset_id) => {
                this.payment_method = data;
                this.deposit.payment_method = data;
                if(asset_id) {
                    this.deposit.asset_id = asset_id;
                    this.deposit.amount = this.currencies.assets.find(item => item.id === asset_id).min_deposit_amount;
                    this.deposit.is_fiat = false;
                    this.createDeposit()
                }
                this.step = 0;
            },
            'setWithdrawalMethod': (data) => {
                this.withdrawal_method = data;
                this.withdrawal.payment_method = data;
                this.step = 0;
            },
            'continue-deposit': (data) => {
                this.deposit.amount = data;
                this.step += 1;
            },
            'selectBank': (data) => {
                this.deposit.bank_id = data;
                this.deposit.currency_id = this.$auth.user.active_currency_id;
                this.deposit.is_fiat = true;
                this.createDeposit()
                this.step += 1;
            },
            'showHistory': (data) => {
                this.show_history = true;
                this.history_hash = data;
            },
            'setActiveBalance': async (data) => {
                let { data: res } = await this.$axios.get(`/private/wallet/currencies/set-active/${data.currency.id}`)
                if(!res.success) {
                    this.$toast.error(res.message)
                    return;
                }
                this.activeBalance = data;
            }
        };

        for (const eventName in eventHandlers) {
            this.$nuxt.$off(eventName);
            this.$nuxt.$on(eventName, eventHandlers[eventName]);
        }
    },
    ...
    methods: {
        back() {
            if (this.step > 0) {
                this.step -= 1;
            } else if (this.payment_method !== null || this.withdrawal_method !== null || this.show_history) {
                this.payment_method = null;
                this.withdrawal_method = null;
                this.show_history = false;
            } else {
        ...
        async createDeposit() {
            try {
                const { data: res } = await this.$axios.post('/private/deposits/create', this.deposit)
                if(!res.success) {
                    this.$toast.error(res.message)
                    return;
                }
                this.depositResponse = res.data;
            } catch (error) {
        ...
        async fetchCurrencies() {
            try {
                const { data: res } = await this.$axios.get('/private/wallet/currencies-list')
                if(!res.success) {
                    console.log('Error');
                    return;
                }
        ...
        async fetchBalances() {
            try {
                const { data: res } = await this.$axios.get('/private/wallet/balances')
                if(!res.success) {
                    console.log('Error');
                    return;
                }
                this.balances = res.data;
                this.activeBalance = this.balances.filter(balance => balance.currency_id === this.$auth.user.active_currency_id)[0];
            } catch (error) {
                console.log('Internal server error');
            }
        }
